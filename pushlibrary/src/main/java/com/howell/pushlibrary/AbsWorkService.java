...
public abstract class AbsWorkService extends Service {
    ...
    public static boolean mShouldAlive = false;
    /**
    ...
    public abstract Boolean shouldStopService();
    ...
    void startService(Intent intent, int flags, int startId) {
        //检查服务是否不需要运行
        Boolean shouldStopService = shouldStopService(intent, flags, startId);
        if (shouldStopService != null && shouldStopService) {Log.e("547","shouldStop  "+shouldStopService); return;}
        //若还没有取消订阅，说明任务仍在运行，为防止重复启动，直接 return
        Boolean workRunning = isWorkRunning(intent, flags, startId);
        if (workRunning != null && workRunning){Log.e("547","wrokRunning ="+workRunning); return;}
        //业务逻辑
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        Log.i("547","abs workService on StartCommand");
